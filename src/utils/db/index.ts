...
import type { PracticeSessionState } from '@/pages/Practice/store/type'
...
export function useSaveWordRecord() {
  ...
  const saveWordRecord = useCallback(
    async ({
      ...
      if (dispatch) {
        if (dbID > 0) {
          dispatch({ type: PracticeActionType.ADD_WORD_RECORD_ID, payload: dbID })
        }
        dispatch({ type: PracticeActionType.SET_IS_SAVING_RECORD, payload: false })
      }
    },
